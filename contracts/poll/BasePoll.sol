...
contract BasePoll {
    ...
    modifier checkTime() {
        require(
            ...
            "is not within time constraints"
        );
        _;
    }
    ...
    function initialize(
        ...
        require(
            _startTime >= now && _endTime > _startTime,
            "is not a valid time constraint"
        );
    ...
    function vote(address voter, bool agree) external checkTime {
        ...
        uint256 maxVoiceWeight = token.totalSupply().div(
            MAX_TOKENS_WEIGHT_DENOM
        );
        voiceWeight = voiceWeight <= maxVoiceWeight
            ? voiceWeight
            : maxVoiceWeight;
