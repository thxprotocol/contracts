...
describe('Test ClaimReward(for), storage/access', function () {
    ...
    it('Claim rewardFor member as non owner', async function () {
        await expect(assetPool.connect(voter).claimRewardFor(0, await owner.getAddress())).to.be.revertedWith(
            'NOT_OWNER',
        );
    });
...
describe('Test ClaimReward(for), flow', function () {
    ...
    beforeEach(
        (_beforeDeployment = async function () {
            ...
            await assetPool.connect(owner).claimRewardFor(0, await owner.getAddress());
