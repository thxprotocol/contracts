...
describe('Test auth', async function () {
    ...
    describe('AssetPool', async function () {
        it('setProposeWithdrawPollDuration', async function () {
            ...
            await expect(assetPool.connect(member).setProposeWithdrawPollDuration(180)).to.be.revertedWith(
                'NOT_MANAGER',
            );
            // gas station
        ...
        it('updateReward', async function () {
            await expect(assetPool.connect(owner).updateReward(0, parseEther('5'), 10)).to.be.revertedWith(
                'NOT_GASSTATION',
            );
            await expect(assetPool.connect(manager).updateReward(0, parseEther('5'), 10)).to.be.revertedWith(
                'NOT_GASSTATION',
            );
            await expect(assetPool.connect(member).updateReward(0, parseEther('5'), 10)).to.be.revertedWith(
                'NOT_GASSTATION',
            );
            // gas station
        ...
        it('claimRewardFor', async function () {
            await expect(assetPool.connect(manager).claimRewardFor(0, await owner.getAddress())).to.be.revertedWith(
                'NOT_OWNER',
            );
            await expect(assetPool.connect(member).claimRewardFor(0, await owner.getAddress())).to.be.revertedWith(
                'NOT_OWNER',
            );
            await expect(assetPool.connect(nonuser).claimRewardFor(0, await owner.getAddress())).to.be.revertedWith(
                'NOT_OWNER',
            );
            await expect(assetPool.connect(manager).claimRewardFor(0, await owner.getAddress())).to.be.revertedWith(
                'NOT_OWNER',
            );
            await expect(assetPool.connect(owner).claimRewardFor(0, await owner.getAddress())).to.be.revertedWith(
                'IS_NOT_ENABLED',
            );
        });
        ...
        it('proposeWithdraw', async function () {
            await expect(assetPool.connect(owner).proposeWithdraw(0, await nonuser.getAddress())).to.be.revertedWith(
                'NOT_GASSTATION',
            );
            await expect(assetPool.connect(manager).proposeWithdraw(0, await nonuser.getAddress())).to.be.revertedWith(
                'NOT_GASSTATION',
            );
            await expect(assetPool.connect(member).proposeWithdraw(0, await nonuser.getAddress())).to.be.revertedWith(
                'NOT_GASSTATION',
            );
            // gas station
        ...
        it('_createWithdrawPoll', async function () {
            expect(() => assetPool._createWithdrawPoll(0, 180, nonuser.getAddress())).to.be.throw(
                'assetPool._createWithdrawPoll is not a function',
            );
        });
        it('_createRewardPoll', async function () {
            expect(() => assetPool._createRewardPoll(0, 180, 5)).to.be.throw(
                'assetPool._createRewardPoll is not a function',
            );
        });
        it('onRewardPollFinish', async function () {
            ...
            await expect(assetPool.connect(manager).onRewardPollFinish(0, 180, 5, false)).to.be.revertedWith(
                'NOT_POLL',
            );
        ...
        it('onWithdrawal', async function () {
            await expect(
                assetPool.connect(owner).onWithdrawalPollFinish(await owner.getAddress(), 1, true),
            ).to.be.revertedWith('NOT_POLL');
            await expect(
                assetPool.connect(manager).onWithdrawalPollFinish(await owner.getAddress(), 1, true),
            ).to.be.revertedWith('NOT_POLL');
            await expect(
                assetPool.connect(member).onWithdrawalPollFinish(await owner.getAddress(), 1, true),
            ).to.be.revertedWith('NOT_POLL');
            // gas station
            tx = await helpSign(
                gasStation,
                assetPool,
                'onWithdrawalPollFinish',
                [await owner.getAddress(), 1, true],
                owner,
            );
            expect(tx.error).to.be.eq('NOT_POLL');
        });
